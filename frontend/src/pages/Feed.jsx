...
export default function Feed() {
  ...
  const [content, setContent] = useState('')
  const [posting, setPosting] = useState(false)
  const [postMsg, setPostMsg] = useState(null) // { type: 'success'|'warn'|'error', text }
  const [showLoginPrompt, setShowLoginPrompt] = useState(false)
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (moderation.status === 'visible') {
        ...
        setPostMsg({ type: 'warn', text: '⚠ Your post is under review by moderators (toxicity score: ' + (moderation.toxicity_score * 100).toFixed(0) + '%).' })
        setContent('')
        setAiWarning(null)
      } else {
        setPostMsg({ type: 'error', text: '✕ Post blocked — harmful content detected (score: ' + (moderation.toxicity_score * 100).toFixed(0) + '%). Please revise.' })
      }
    } catch (err) {
      setPostMsg({ type: 'error', text: err.response?.data?.error || 'Failed to post. Try again.' })
    } finally {
    ...
    <div className={styles.page}>
      ...
      <div className={styles.container}>
        ...
        <div className={`${styles.composer} ${getWarningClass()}`}>
          ...
          {/* Real-time AI warning */}
          {aiWarning && (
            <div className={styles.aiWarning}>
              <span>⚠</span>
              <div>
                <strong>This message may be harmful</strong>
                <p>Toxicity score: {(aiWarning.score * 100).toFixed(0)}% — {aiWarning.label}. Your post may be withheld or blocked by our AI moderator.</p>
              </div>
            </div>
          )}
